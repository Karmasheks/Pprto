...
import { BarChart2, Users, ChartBar, Wrench, Clipboard, X } from "lucide-react";
        ...
        <div className="flex-shrink-0 flex border-t border-gray-200 p-4 dark:border-gray-700">
          <a href="#" className="flex-shrink-0 group block">
            <div className="flex items-center">
              ...
              <div className="ml-3">
                <p className="text-base font-medium text-gray-700 dark:text-gray-300">{user?.name || "Никита"}</p>
                <p className="text-sm font-medium text-gray-500 dark:text-gray-400">{user?.role === "admin" ? "Администратор" : user?.role === "manager" ? "Менеджер" : user?.role || "Администратор"}</p>
              </div>
            </div>
          </a>
        </div>
